...
Handles Team Members, Footer Settings, and Page Content management
"""
...
from apps.platformadmin.models import TeamMember, FooterSettings, PageContent
from apps.platformadmin.forms import TeamMemberForm, FooterSettingsForm, PageContentForm
...
def team_members_list(request):
    ...
    return render(request, 'platformadmin/feature_settings/team_members_list.html', context)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ActivityLog.log_action(
                ...
                str(team_member.id),
                team_member.name,
                new_values={'name': team_member.name, 'designation': team_member.designation}
            )
            
            messages.success(request, f'Team member "{team_member.name}" created successfully!')
            return redirect('platformadmin:team_members_list')
    else:
        form = TeamMemberForm()
    
    context = {'form': form}
    return render(request, 'platformadmin/feature_settings/team_member_form.html', context)
...
def team_member_edit(request, member_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            team_member.subject = form.cleaned_data.get('subject', '')
            team_member.years_of_experience = form.cleaned_data['years_of_experience']
            team_member.bio = form.cleaned_data.get('bio', '')
            team_member.email = form.cleaned_data.get('email', '')
            team_member.phone = form.cleaned_data.get('phone', '')
            team_member.linkedin_url = form.cleaned_data.get('linkedin_url', '')
            team_member.twitter_url = form.cleaned_data.get('twitter_url', '')
            team_member.facebook_url = form.cleaned_data.get('facebook_url', '')
            team_member.is_active = form.cleaned_data.get('is_active', True)
            team_member.display_order = form.cleaned_data['display_order']
            
            # Update photo if provided
            if 'photo' in request.FILES:
                team_member.photo = form.cleaned_data['photo']
            ...
            ActivityLog.log_action(
                ...
                new_values={'name': team_member.name, 'designation': team_member.designation, 'is_active': team_member.is_active}
            )
        ...
        initial_data = {
            ...
            'years_of_experience': team_member.years_of_experience,
            'bio': team_member.bio,
        ...
        form = TeamMemberForm(initial=initial_data)
...
def team_member_delete(request, member_id):
    ...
    if request.method == 'POST':
        ...
        ActivityLog.log_action(
            ...
            str(team_member.id),
            name,
            old_values={'name': name, 'designation': team_member.designation}
        )
    ...
    return render(request, 'platformadmin/feature_settings/team_member_confirm_delete.html', context)
